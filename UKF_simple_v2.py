...
class UnscentedKalmanFilter:
    def __init__(self, state_dim, measurement_dim, process_noise_cov, measurement_noise_cov):
        ...
        self.cross_cov_meas = np.zeros([2,4])
    ...
    def _generate_sigma_points_measurement(self):
        ...
        self.sigma_points_measurement[:,0] = np.hstack([self.state_estimate,self.state_estimate,self.state_estimate])
        self.weights_measurement_update[0] = self.lamb_meas/(self.total_dim+self.lamb_meas)
        for i in range(self.total_dim):
            self.sigma_points_measurement[:,2*i+2] = self.sigma_points_measurement[:,0] + np.sqrt(self.total_dim+self.lamb_meas)*S[i]*U[:,i]
            self.sigma_points_measurement[:,2*i+1] = self.sigma_points_measurement[:,0] - np.sqrt(self.total_dim+self.lamb_meas)*S[i]*U[:,i]
    ...
    def _calculate_sigma_covariance(self):
        ...
        mu_z = np.reshape(mu_z,[6,1])
        self.measurement_transform_cov = self.weights_measurement_update[0] * (self.sigma_points_measurement[:,0]-mu_z).T@(self.sigma_points_measurement[:,0]-mu_z)
        for i in range(self.total_dim):
            self.measurement_transform_cov += self.weights_measurement_update[2*i+2] * (self.sigma_points_measurement[:,2*i+2]-mu_z)@(self.sigma_points_measurement[:,2*i+2]-mu_z).T
            self.measurement_transform_cov += self.weights_measurement_update[2*i+1] * (self.sigma_points_measurement[:,2*i+1]-mu_z)@(self.sigma_points_measurement[:,2*i+1]-mu_z).T
        self.measurement_transform_cov += (1-self.alpha_meas**2+self.beta_meas**2)*(self.sigma_points_measurement[:,0]-mu_z)@(self.sigma_points_measurement[:,0]-mu_z).T
    ...
    def _generate_sigma_points_time(self):
        ...
        for i in range(self.state_dim):
            self.sigma_points_time[:,2*i+2] = self.sigma_points_time[:,0] + np.sqrt(self.state_dim+self.lamb_time)*S[i]*U[:,i]
            self.sigma_points_time[:,2*i+1] = self.sigma_points_time[:,0] - np.sqrt(self.state_dim+self.lamb_time)*S[i]*U[:,i]
    ...
    def _calculate_state_cov_time(self):
        ...
        self.state_cov = self.weights_time_update[0] * (self.sigma_points_time[:,0]-mu_z)@(self.sigma_points_time[:,0]-mu_z).T
        for i in range(self.state_dim):
            self.state_cov += self.weights_time_update[2*i+2] * (self.sigma_points_time[:,2*i+2]-mu_z)@(self.sigma_points_time[:,2*i+2]-mu_z).T
            self.state_cov += self.weights_time_update[2*i+1] * (self.sigma_points_time[:,2*i+1]-mu_z)@(self.sigma_points_time[:,2*i+1]-mu_z).T
        self.state_cov += (1-self.alpha_meas**2+self.beta_meas**2)*(self.sigma_points_time[:,0]-mu_z)@(self.sigma_points_time[:,0]-mu_z).T
